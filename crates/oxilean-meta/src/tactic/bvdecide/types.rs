...
pub struct CdclSolver {
    /// The clause database.
    pub(super) clause_db: ClauseDb,
    /// Current assignment.
    pub(super) assignment: Assignment,
    /// VSIDS branching heuristic.
    pub(super) vsids: VsidsScorer,
    /// Watch lists: for each literal, list of clause indices watching it.
    pub(super) watch_lists: HashMap<i32, Vec<usize>>,
    /// Watched literal info per clause.
    pub(super) watched: HashMap<usize, WatchedInfo>,
    /// Number of variables.
    pub(super) num_vars: usize,
    /// Statistics.
    pub(super) stats: CdclStats,
    /// Configuration.
    pub(super) config: CdclConfig,
    /// Resolution chain for UNSAT proof.
    pub(super) proof_chain: Vec<ResolutionStep>,
}
impl CdclSolver {
...
pub struct GoalAnalyzer {
    ...
    pub(super) var_map: HashMap<u64, (String, BitWidth)>,
    /// Next variable name counter.
    pub(super) next_var: u32,
    /// Known bit-vector type widths by name.
    pub(super) known_bv_types: HashMap<String, BitWidth>,
}
impl GoalAnalyzer {
...
pub struct Assignment {
    ...
    pub(super) levels: Vec<Option<u32>>,
...
pub struct BvEncoder {
    ...
    pub(super) expr_cache: HashMap<u64, Vec<SatVar>>,
    /// Next expression ID for caching.
    pub(super) next_expr_id: u64,
    /// Named BV variable map: variable name -> SAT variable bits.
    pub(super) named_vars: HashMap<String, Vec<SatVar>>,
}
impl BvEncoder {
...
pub struct VsidsScorer {
    ...
    pub(super) order_dirty: bool,
    /// Sorted variable order (highest activity first).
    pub(super) order: Vec<SatVar>,
}
impl VsidsScorer {
